...
specified = data[(data['ORIGIN'] == 'JFK') & (data['FL_DATE'] == '2012-01-21') & \
                 (data['CRS_DEP_TIME'] <= 1200) & (data['CRS_DEP_TIME'] >= 900)]
...
px.box(specified['DEP_DELAY'].tolist(), points='all').update_layout(xaxis_title = 'Departure Delays')

departure = pd.merge(specified[['FL_DATE', 'CRS_DEP_TIME', 'DEP_DELAY', 'DEST']], coordinates, left_on = 'DEST', right_on='iata')

px.scatter(x=pd.to_datetime(departure['FL_DATE'] + departure['CRS_DEP_TIME'].astype(str), format='%Y-%m-%d%H%M'), y = departure['DEP_DELAY'].tolist(), \
           color = departure['DEP_DELAY'] > 0, color_discrete_sequence=['red', 'green'], labels = {'color': 'Delay', 'x': 'Scheduled Departure Time', 'y': 'Delay in Departure Time'}, \
            hover_name=departure['airport']).update_layout(\
    xaxis_title = 'Scheduled Departure Time', yaxis_title = 'Delay in Departure Time', showlegend = False)
#    dtick="M48",
#    tickformat="%H%M")

#breakdown of delay cause for arrival
px.bar((specified[['CARRIER_DELAY', 'WEATHER_DELAY', 'NAS_DELAY', 'SECURITY_DELAY', 'LATE_AIRCRAFT_DELAY']] \
> 0).sum()).update_layout(xaxis_title = 'Cause of delay', yaxis_title = 'Number of delay', showlegend = False)

arrival = specified.groupby('DEST')['ARR_DELAY'].aggregate(['count', 'mean']).reset_index().fillna(0)
...
for i in range(len(arrival)):
    fig.add_trace(
        go.Scattergeo(
            ...
            lon = [coordinates[coordinates['iata'] == 'JFK']['long'].iloc[0], full['long'][i]],
            lat = [coordinates[coordinates['iata'] == 'JFK']['lat'].iloc[0], full['lat'][i]],
            mode = 'lines',
