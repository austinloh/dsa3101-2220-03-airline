...
layout = html.Div(
    [
        ...
        html.Br(),
        html.P(
            "Hover over the visualization to see the CRSDepTime and corresponding average delay for that timing"
        ),
        html.P(
            "Example 1: (440, 76) means at 4:40 am, the average delay is 76 minutes."
        ),
        html.P(
            "Example 2: (1645, 25) means at 4:45 pm, the average delay is 25 minutes."
        ),
        html.P(
            "You may notice some flat lines that occur over regular intervals, that is due to time not having minutes \
           between 60 and 99, as 24 hour time jumps from XX59 to XX00."
        ),
        html.Br(),
        html.Label(
            ["Select Delay Type"], style={"font-weight": "bold", "text-align": "center"}
        ),
        dcc.Dropdown(
            id="delay_type",
            options=[
...
def update_graph(option_selected):
    ...
    if option_selected == "arrival":
        ...
        fig.add_trace(
            go.Scatter(
                x=temp_arr_df["CRSDepTime"],
                y=temp_arr_df["ArrDelay"],
                name="Arrival Delay",
                line=dict(color="firebrick", width=3),
            )
        )
        fig.update_layout(
            title="Average Arrival Delays With CRSDepTime (Scheduled Departure Time)",
        ...
        fig.add_trace(
            go.Scatter(
                x=temp_dep_df["CRSDepTime"],
                y=temp_dep_df["DepDelay"],
                name="Departure Delay",
                line=dict(color="royalblue", width=3),
            )
        )
        fig.update_layout(
            title="Average Departure Delays With CRSDepTime (Scheduled Departure Time)",
            xaxis_title="CRSDepTime (in 24 hour Format)",
            yaxis_title="Delay (in minutes)",
        )
        ...
        fig.update_layout(
            title="Arrival & Departure Delays With CRSDepTime (Scheduled Departure Time)",
            xaxis_title="CRSDepTime (in 24 hour Format)",
            yaxis_title="Delay (in minutes)",
        )
    return [fig]
